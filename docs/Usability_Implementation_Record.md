...
- The owner-review contrast correction makes the preference visible where work starts: Overview, Devices, Profiles, Axes, Buttons, Automation, and Signal Flow share a page-local contextual callout. Guided gives the next decision a highlighted, readable explanation; Full keeps a compact editing context instead of a generic level badge. The existing Overview connection/additional-attention and Devices virtual-output/isolation disclosures now use the same policy: untouched detail starts open in Full and remains explicitly reachable in Guided. An exact deep link still opens its existing target.
...
| --- | --- | --- | --- |
| Overview, Devices, Axes, Buttons, Profiles, Automation, Signal Flow | A highlighted page-local next decision makes the current choice readable before the existing controls; all actions and deep links remain visible. Overview/Devices technical context remains a named disclosure. | A compact page-local editing context accompanies the same destinations, ordering, editors, graph, and actions. Untouched Overview connection/additional-attention and Devices output/isolation context starts open. | No duplicate page tree, identity remapping, activation, or topology change. |
...
- The Release `app_qml_startup_tests` lifecycle journey drives the actual Settings controls, observes meaningful Guided/Full defaults on Diagnostics, Adaptive Response, Overview, and Devices, and verifies the shared page-local callout on the latter two surfaces. It compares the canonical Flight Deck configuration snapshot plus setup task before/after. It also changes guidance while the accepted Step 2 assistant is open with checkbox drafts, and while the existing repair confirmation is open; task, drafts, active Rig/Profile, mapping request, and configuration stay unchanged.
...
- The side-by-side native review candidate is `C:\hotas-builds\usability-setup-guidance\Release\HOTAS BF6 Pass C Guidance Contrast Review.exe --isolated-presentation` (SHA-256 `01A70D0813E12710FEEB1B3D6BFBC3480AF7EAB1B22C031C5ABC633D408674B3`). It launched with a visible `HOTAS BF6` main window; it is presentation-only and does not establish a physical-controller, driver, or owner-acceptance verdict.
...
## Pass C beginner workflow and shared-input correction (2026-09-24; unmerged)

- Axes and Buttons now share `FlightDeckInputStatusCard.qml`. Both editors project the existing selected-controller, connection, output, and mapping facts into the same heading, two status pills, explanatory body, and at-most-two-action hierarchy. The state covers scan, choose, exact-controller setup, offline editing, no relevant controls, output attention, first mapping, and ready work without introducing a second controller/configuration authority. Offline mappings remain editable; scanning, choosing, setup, and checking route to the existing commands.
- The persistent top-bar controller/profile selectors remain readable in compact headers instead of collapsing to unlabeled glyph controls. The direct controller picker opens the existing selector popup and does not activate a Rig or change Mapping. The header's secondary controller/appearance shortcuts defer before either context selector does.
- Guided Axes calls a route **assigned** only when it is configured and labels live input as **input detected** only after observation. The J/S mathematical value remains **Curve strength** and is Full-only instead of being presented as an unrelated Guided sensitivity control. Guided conflict prompts offer the exact existing Full destination for a mix decision; Buttons preserves the same behavior for its Signal Flow target.
- Guided Buttons keeps ordinary direction mapping and exposes an ordinary whole-hat-to-game-POV action using the existing native POV command. Native descriptor terminology, profile triggers, rules, and automation remain Full-only.
- The Guided Devices page calls a verified unassigned controller **Set up this controller** and sends it to the existing durable assistant rather than showing a dead-end message. It provides a setup summary (members, profile, status, check, profile creation, and explicit use); its creation dialog only proposes an output when exactly one exists and otherwise keeps the user at an explicit Full-only decision. A newly created setup still leads directly to the existing Profile workflow without implicit activation.
- Guided Profiles has one Create and one Import/Export entry at a time, preserves the existing native **More** menu for Rename/Duplicate/Export, and suppresses category administration. A clean Overview suppresses the generic Attention card rather than reporting a non-issue.
- Basic setup retains the accepted copy-profile and required-member checkboxes. It removes the redundant Guided **First controller** topology choice, labels the two remaining paths as separate/current setup, and displays connection/profile-group selectors only when there is a real multiple-choice decision. A lone canonical output/category is shown as the selected fact; no first-list fallback is used. Selecting a Full-only connection opens the existing Full confirmation while retaining the setup task.
- Current local Release evidence: a cached all-target build completed, then `ctest --test-dir C:\hotas-builds\usability-setup-guidance -C Release --output-on-failure` passed **19/19** on 2026-09-24. The focused `app_qml_guidance_qualification_tests` passed in 33.65 seconds. It now drives the readable direct controller picker by pointer and keyboard with a configuration snapshot invariant; validates Guided's six-page/Full-only recovery; opens the four-stage assistant, selects Step 1 by keyboard, advances Step 2 by pointer without replacing its task, exercises both accepted checkbox roles, and checks the shared Axes/Buttons preparation cards. The full suite also passed `mapping_hot_path_benchmark` with zero reported hot-path allocations in every listed scenario.
- These are QML and synthetic mapper results. They do not establish real controller acquisition, vJoy/HidHide behavior, driver repair, native typography, or owner acceptance. Offscreen captures remain structural evidence only.
