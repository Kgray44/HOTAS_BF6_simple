...
| Controller inventory and setup | Settings, controller setup dialog, `ControllerManager` and readiness model | Devices & setup; health summary first, repair and details contextual | Primary; connected/new/verified/offline/ambiguous device, calibration required, HidHide/vJoy attention, repair/progress/rollback/reconnect |
...
| Dialogs, flyouts, confirmations, and notifications | Standard/Legacy shells and page components | Preserve existing behavior while page bodies are redesigned in later phases | Contextual; learning, conflicts, create/rename/delete, setup/recovery, import preview, tooltips, empty/loading/error/validation states |
