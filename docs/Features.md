...
A dedicated DirectInput-to-vJoy worker owns bounded single-device and Device Rig report processing so controller output is not scheduled by the UI.
...
- Fixed-capacity Device Rig sessions independently acquire known physical inputs and write each configured virtual output without report-path identity lookup, UI work, or allocation.
...
Physical buttons are discovered from the active controller and routed to available vJoy buttons with explicit, user-confirmed ownership or shared-output fan-in.
...
- Replace, Ignore, and Cancel make destination conflicts explicit: Replace atomically swaps a valid prior output, Ignore intentionally shares an output, and Cancel leaves all routes unchanged.
...
- Clearly clickable Learn Input actions freeze the selected virtual destination before a physical button is detected.
- Quick Map — Buttons uses active-profile destinations rather than unused vJoy capacity, re-arms until held buttons are released, and preserves completed steps on Exit.
...
- Automatic Activation uses one ordered, hardware-aware Category → Profile → Device Rig → Virtual Output resolver. Profiles persist Preferred, Fallback, or Manual Only policy; a valid current pair remains stable, and Manual Only is never selected automatically.
- Direct manual profile activation creates a runtime/session override that can be ended with Resume Automatic and clears on game-context change or restart. Profiles without a Device Rig remain manually selectable but are ineligible for automatic selection until assigned a complete, verified rig.
...
- Same-Rig, same-layout Profile Hold/Toggle changes use compiled runtime state; incompatible Device Rig targets are rejected, and changing layouts is an explicit neutral/release/reacquire control-plane transition.
...
- Portable .hbf6profile and .hbf6pack files validate bounded versioned JSON before preview and atomic import; hardware descriptors and calibration are excluded by default, HidHide ownership is stripped, and a source Device Rig is left unassigned with a Device Rig assignment required message rather than matched by name.
...
HOTAS BF6 discovers and remembers physical DirectInput controllers as durable, explicit inputs and groups them into reusable Device Rigs outside report processing.
...
- Smart inventory reconciliation feeds complete, partial, ambiguous, and verification-needed rig state to the Activation Resolver. It never independently changes the active rig or infers identity from a friendly name.
...
- Activation Resolver reason, selected Category/Profile/Device Rig/Virtual Output, stable retention, manual override, and actionable ineligibility blockers are published as low-frequency control-plane state.
...
- Flight Deck Settings includes Guidance level. Guided keeps the common task direct and starts optional Axes tuning, Button relationships, Profile association, Automation behavior, Curve details, and Signal Flow inspector controls closed with local decision help; Full starts those selected optional editor groups open. Devices deep links reveal their exact target only for the current view. The choice never changes mappings, Profiles, Device Rigs, automation, repair authority, themes, text size, or activation.
- A new installation can choose Guided or Full in the themed first-use dialog; Skip for now records Guided. Existing installations without this preference retain a non-intrusive Full-compatible default, and a disconnected controller or empty rig inventory is never treated as first use.
- Advanced sections remain available in both guidance levels. An explicit local open/closed choice takes precedence over the general level and survives navigation, restart, and a later level change.
...
- Flight Deck Profiles orders automatic configurations and exposes Device Rig readiness and Preferred/Fallback/Manual Only policy; Overview and Diagnostics show the active resolver explanation and Resume Automatic action.
...
- Automatic Activation may use the established virtual-output visibility transaction, but never hides, shows, adopts, or changes a physical controller's HidHide membership.
...
One shared Device Rig setup and verification system explains a single controller or a full multi-device chain without moving setup work into real-time mapping.
