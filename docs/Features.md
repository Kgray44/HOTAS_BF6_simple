...
- Direct manual profile activation creates an explicit override that can be ended with Resume Automatic. Profiles without a Device Rig remain manually selectable but are ineligible for automatic selection until assigned a complete, verified rig.
...
- Schema 25 persists Profile automatic-selection policy plus an explicit manual override. Schema-24-and-earlier categories migrate their saved default (or first profile) to Preferred and the remaining profiles to deterministic Fallback order.
