...
| Activity lifecycle selection and Inspector navigation | PASS, narrow R9 history-label clarity | The owner selected a completed check's historical `CHECK STARTED` row in R9 and reported that the new `STATE AT START: RUNNING` label and later `CHECK COMPLETED` outcome made the lifecycle history clear. This confirms the status wording only; a separate two-row/arrow-navigation recheck remains required. |
