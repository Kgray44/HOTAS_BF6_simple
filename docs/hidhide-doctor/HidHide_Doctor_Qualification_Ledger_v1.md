...
| bounded hostile input campaign | Fixed seed `0x5AFEF00D`; 64 malformed inputs each for integration contexts/results, journals, helper frames, package metadata, and Safe-to-Share report/evidence composition. The direct domain suite exited 0 in this closure run. The immutable final stage also passed the direct Unicode/metacharacter report-destination contract with no shell interpretation. | FixtureQualified plus narrow Qualified path contract | Bounded deterministic parser coverage, not sustained fuzzing, installed-package abuse, or a resource-pressure campaign. |
