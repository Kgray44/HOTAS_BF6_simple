...
| Report export completion | Qt event-loop domain test exported Markdown to its requested temporary destination, verified the created nonempty file, and verified the completion status identifies that native path. | Executed, domain fixture | An owner desktop export/reopen retest remains required; no native file-picker or shell observation is claimed here. |
