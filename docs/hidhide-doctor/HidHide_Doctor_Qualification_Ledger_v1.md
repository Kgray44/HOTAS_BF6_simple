...
| Report export completion | Qt event-loop domain test exported Markdown through the same typed local `file:///` URL route used by the dialogs, verified the created nonempty file and its native completion path, and contract-checked the file/folder handoffs plus visible `Exporting…` and result-status bindings. The configured local CTest suite passed 17/17 after this correction. | Executed, domain and source-contract fixture | An owner desktop export/reopen retest remains required; no native file-picker or shell observation is claimed here. |
