...
| Staged HOTAS Doctor entry points | Owner manually confirmed that **Devices**, **Diagnostics/App Health**, and **Flight Deck → Diagnostics** each opened or focused the paired pre-R2 staged Doctor. The Devices launch was independently observed to start `HidHide Doctor.exe` from the same stage with a bounded `--integration-context`; the paired staged process count remained one. R2 has not yet received this owner-native repeat. | Qualified, historical narrow | Covers the three deliberate launch surfaces and single-instance behavior only. R2 requires its own owner-native check; it is not broad visual, DPI, screen-reader, hardware, or repair qualification. |
