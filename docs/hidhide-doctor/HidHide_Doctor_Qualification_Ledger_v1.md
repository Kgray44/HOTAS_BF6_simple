...
| final local CTest rerun | A fresh sequential CTest run against the final-source build passed all 18 registered tests in 260.14 seconds with `--timeout 180`: the five Doctor tests, mapper/launcher/updater/font contracts, `app_backend_startup_tests` in 13.20 seconds, `app_qml_startup_tests` in 157.25 seconds, and the theme test. | Qualified | The explicit 180-second CTest bound prevents a local test process from being mistaken for a normal indefinitely-running owner process. The result is automated/offscreen evidence, not native owner acceptance. |
