...
| final local CTest rerun | A fresh sequential CTest run against the R2-source build passed all 18 registered tests in 210.85 seconds with `--timeout 180`: the five Doctor tests, mapper/launcher/updater/font contracts, `app_backend_startup_tests` in 12.64 seconds, `app_qml_startup_tests` in 126.34 seconds, and the theme test. The test-only startup build no longer starts the real mapper, eliminating a shutdown hang after its assertions had passed; production mapper startup is unchanged. | Qualified | The explicit 180-second CTest bound prevents a local test process from being mistaken for a normal indefinitely-running owner process. The result is automated/offscreen evidence, not native owner acceptance. |
