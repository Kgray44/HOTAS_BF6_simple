...
| Keyboard activation and exposed button roles | Source `f2531bd` makes the shared custom Doctor button and segmented-control segments expose `Accessible.Button` and activate with Return/Enter. The native-QML layout regression exercises forward Tab/Space, reverse Shift+Tab/Return, and Escape restoration of a maximized pane; it passed warning-free. | FixtureQualified | This proves the focused QML interaction contract only. A native keyboard-only owner pass and screen-reader observation remain required. |
...
| Final-stage hostile report destination | The immutable `HidHideDoctor-v1-RC1-2.6.2-R2` candidate passed a direct-process contract: a Unicode destination containing spaces, `&`, `%`, `;`, and an apostrophe created valid JSON with exit 0. A path whose parent was a regular file exited 3 and created no report. The contract uses `ProcessStartInfo.ArgumentList`, so no shell interpreted the destination. | Qualified, narrow | This covers the standalone headless `--report` argument route only; it is not a broad installer, RunOnce, cache, or helper-path qualification. |
...
| Updater component pairing and rollback | Stage validation requires mapper, Doctor, and helper versions to agree. `launcher_core_tests` exercises failed installer, interrupted replacement, missing/invalid package, and candidate-startup failures; each restores the complete prior fixture package and preserves an external configuration sentinel. The isolated release acceptance script now requires the installed Doctor and helper and runs the standalone Doctor startup smoke after every resulting candidate clean/upgrade/recovery installation. Its v2.5.0 pre-upgrade fixture may deliberately lack the later Doctor components, but the candidate replacement may not. | FixtureQualified | The strengthened script has not yet executed in an isolated release workflow; no installed-product update or recovery/retry result is claimed. |
...
| Real Windows scaling, keyboard navigation, accessibility names/roles, and in-app control-style review | A. LOCAL RC BLOCKER | Partially FixtureQualified / Partially Qualified | The owner reported the requested native display review looked good. Source `f2531bd` now proves the focused Tab/Shift+Tab/Space/Return/Escape contract and explicit custom-button roles, but scale-by-scale/page-by-page capture, keyboard-only traversal, and accessibility-name/role evidence remain required. Screen-reader remains explicitly unqualified unless actually exercised. |
