...
| Final-stage hostile report destination | The immutable `HidHideDoctor-v1-RC1-2.6.2` candidate passed a direct-process contract: a Unicode destination containing spaces, `&`, `%`, `;`, and an apostrophe created valid JSON with exit 0. A path whose parent was a regular file exited 3 and created no report. The contract uses `ProcessStartInfo.ArgumentList`, so no shell interpreted the destination. | Qualified, narrow | This covers the standalone headless `--report` argument route only; it is not a broad installer, RunOnce, cache, or helper-path qualification. |
...
| Exact-stage process/startup/scan baseline | On immutable `HidHideDoctor-v1-RC1-2.6.2`, nine `--startup-smoke` children had a 553.435 ms median (515.728–595.093 ms); nine headless report serializations had a 295.748 ms median (280.710–383.385 ms). A five-second idle observation of the protected interactive Doctor measured 0.02% CPU across 16 logical processors, 163,213,312 B working set, and 149,762,048 B private memory. | Qualified, process-level | The harness does not flush the Windows file cache, measure first visible progress, native input response, or screen-reader output. |
| controlled CPU contention | On immutable `HidHideDoctor-v1-RC1-2.6.2`, three real read-only `--scan-smoke` samples per level all exited 0 without a timeout: idle median 1388.309 ms (1378.894–1403.981), moderate synthetic CPU median 1478.011 ms (1425.716–1484.466), and heavy synthetic CPU median 1733.912 ms (1696.503–1796.992). Heavy child-process peaks were 125,251,584 B working set and 132,849,664 B private memory. The protected interactive Doctor remained running. | Qualified, process-level | Bounded synthetic CPU load and process timing only; it is not owner-perceived interaction latency or an input-overload verdict. |
...
| final local CTest rerun | The final-source focused Doctor CTest set passed 5/5: domain, deep repair, standalone startup, path-boundary contract, and layout. An earlier fresh sequential rerun passed targets 1–14, then `app_backend_startup_tests` remained active for 339 seconds with no configured CTest timeout or error output. Only the agent-launched CTest/test child tree was stopped; the live mapper and Doctor were preserved. | NO NEW LOCAL FULL-CTEST VERDICT | The focused final-source set passes, but it does not create a new full-suite pass. The earlier 17/17 result on the unchanged pre-Unicode-path product head is historical evidence only. |
